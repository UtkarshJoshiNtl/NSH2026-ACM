"""Conjunction detection with Brent TCA refinement and Chan probability of collision."""
...
def _brent_minimise(f, a: float, b: float, tol: float = 0.1) -> float:
    """Brent's method to minimise f over [a, b]."""
    GOLDEN = 0.3819660
    x = w = v = a + GOLDEN * (b - a)
    fx = fw = fv = f(x)
    d = e = 0.0
...
def _chan_pc(miss_dist_km: float, sigma_r_km: float,
    ...
    """Chan's 1997 probability of collision (circular encounter approximation)."""
    r = PcResult()
...
class ConjunctionDetector:
    ...
    def detect(
        ...
        """KDTree broad-phase cull, temporal sweep, then Brent TCA refinement."""
        if not sat_states or not debris_states:
            return []

        # Broad phase: KDTree cull of pairs on opposite sides of Earth
        sat_pos = [s[:3] for s in sat_states]
        deb_pos = [d[:3] for d in debris_states]
        # Conservative radius: ~15 km/s × lookahead, capped at Earth diameter
        ...
        # Import from accelerator (avoids circular dependency)
        from .accelerator import propagate_batch_full_history
        # Only pass mjd0 for lunisolar perturbations if provided
        ...
        # Empirical position covariance from TLE age
        ...
        for sat_idx, candidate_list in enumerate(candidates):
            for deb_idx in candidate_list:
                ...
                # Only refine if under ADVISORY threshold
                if min_dist >= ADVISORY_DISTANCE:
                    continue

                # Brent refinement within ±1 step of coarse TCA
                ...
                def dist_at_t(t: float) -> float:
                    # Start from nearest pre-propagated step
