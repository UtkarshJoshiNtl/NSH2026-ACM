...
async def create_simulation(
    ...
    db_sim = Simulation(
        user_id=user["user_id"], name=sim_data.name, simulation_time=0.0
    )
    db.add(db_sim)
    db.commit()
    db.refresh(db_sim)
...
async def list_simulations(
    ...
    simulations = (
        db.query(Simulation)
        .filter(Simulation.user_id == user["user_id"], Simulation.is_active == True)
        .all()
    )
...
async def get_simulation(
    ...
    sim = (
        db.query(Simulation)
        .filter(Simulation.id == simulation_id, Simulation.user_id == user["user_id"])
        .first()
    )
