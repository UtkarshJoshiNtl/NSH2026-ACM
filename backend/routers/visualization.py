...
async def get_snapshot():
    ...
    if debris:
        ...
        debris_cloud = [
            [
                deb.id,
                round(float(geo_array[i][0]), 3),
                round(float(geo_array[i][1]), 3),
                round(float(geo_array[i][2]), 1),
            ]
    ...
    return {
        ...
        "pending_burns": (
            state_mgr.get_context("default").scheduled_burns
            if state_mgr.get_context("default")
            else []
        ),
    }
