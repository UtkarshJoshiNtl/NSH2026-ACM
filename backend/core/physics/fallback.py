...
def rk4_py_drag(state: tuple, dt: float, area: float = 0.1, mass: float = 100.0, cd: float = 2.2) -> tuple:
    ...
    def acceleration(r, v):
        ...
        if altitude < 1000:  # Only apply drag below 1000 km
            ...
            a_drag = [-drag_accel * v[i] / v_mag for i in range(3)] if v_mag > 0 else [0, 0, 0]
        else:
            a_drag = [0, 0, 0]
