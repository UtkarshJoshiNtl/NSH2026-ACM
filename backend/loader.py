...
def load_initial_state_from_disk(state_mgr):
    ...
    if os.path.exists(sats_path):
        ...
        logger.warning(f"Satellites file not found: {sats_path} - starting with empty satellite state")
    ...
    if os.path.exists(debris_path):
        ...
        logger.warning(f"Debris file not found: {debris_path} - starting with empty debris state")
